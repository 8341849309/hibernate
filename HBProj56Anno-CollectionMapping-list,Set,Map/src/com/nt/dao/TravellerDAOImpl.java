...
public class TravellerDAOImpl implements ITravellerDAO {
	...
	public void save() {
		...
		try (Session ses = HibernateUtils.getSession()) {
			...
			List<String> nickNames=List.of("Venkateswarlu","Rama","Krishna","Yudhistir");
			...
			Traveller tr = new Traveller("Venkateswarlu", dob, nickNames, phones, placesVisited);
	...
	public void load() {
		...
		try (Session ses = HibernateUtils.getSession()) {
			...
			list.forEach(tr -> {
				...
				System.out.println("Traveller Id:: " + tr.getTravellerId() + " Traveller Name:: " + tr.getTavellerName()
						+ " DOB:: " + tr.getDob());

				Set<Long> phones = tr.getPhones();
				phones.forEach(System.out::println);

				List<String> names=tr.getNickNames();
